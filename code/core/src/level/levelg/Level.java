...
public class Level {
    ...
    public List<Node> getShortestPath(Node start, Node goal) {
        ...
        for (List<Node> l : allPaths)
            if (l.size() < shortestPath.size())
                shortestPath = l;
        return shortestPath;
    }
    ...
    public boolean isRoomReachableWithout(Node start, Node goal, Node avoid) {
        ...
        for (List<Node> list : allPaths)
            if (list.contains(avoid))
                pathWithoutAvoid.remove(list);
     ...
     * @param paths
     */
    private void graph_search(Node currentNode, List<Node> marked, Node goal, List<List<Node>> paths) {
